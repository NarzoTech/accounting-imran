...
@section('title')
    <title>{{ __('Edit Container') }}</title>
@endsection

@section('admin-content')
    <form action="{{ route('admin.container.update', $container->id) }}" method="post" id="create_form"
            ...
            <div class="col-md-7 col-lg-8">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <h4 class="section_title">{{ __('Edit Container') }}</h4>
                    </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <x-admin.form-input id="container_type" name="container_type"
                                        ...
                                        value="{{ old('container_type', $container->container_type) }}" :required="true" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <x-admin.form-input id="port_of_loading" name="port_of_loading"
                                        ...
                                        value="{{ old('port_of_loading', $container->port_of_loading) }}" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <x-admin.form-input id="port_of_discharge" name="port_of_discharge"
                                        ...
                                        value="{{ old('port_of_discharge', $container->port_of_discharge) }}" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <x-admin.form-input type="date" id="estimated_departure" name="estimated_departure"
                                        ...
                                        value="{{ old('estimated_departure', $container->estimated_departure?->format('Y-m-d')) }}" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <x-admin.form-input type="date" id="estimated_arrival" name="estimated_arrival"
                                        label="{{ __('Estimated Arrival') }}"
                                        value="{{ old('estimated_arrival', $container->estimated_arrival?->format('Y-m-d')) }}" />
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <x-admin.form-input type="date" id="actual_arrival" name="actual_arrival"
                                        label="{{ __('Actual Arrival') }}"
                                        value="{{ old('actual_arrival', $container->actual_arrival?->format('Y-m-d')) }}" />
                                </div>
                            </div>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    <x-admin.form-textarea id="remarks" name="remarks" label="{{ __('Remarks') }}"
                                        value="{{ old('remarks', $container->remarks) }}" />
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card mt-5">
                    ...
                    <div class="card-body">
                        <div id="product_items_wrapper">
                            @foreach ($container->products as $index => $containerProduct)
                                <div class="row g-2 product-item align-items-end">
                                    <div class="col-md-8">
                                        <x-admin.form-select name="products[{{ $index }}][product_id]"
                                            label="{{ __('Product') }}" class="select2" :required="true">
                                            <x-admin.select-option value="" text="{{ __('Select Product') }}" />
                                            @foreach ($products as $product)
                                                <x-admin.select-option :value="$product->id" :selected="old(
                                    ...
                                    <div class="col-md-3">
                                        <x-admin.form-input type="number" min="1"
                                            name="products[{{ $index }}][quantity]" label="{{ __('Quantity') }}"
                                            value="{{ old('products.' . $index . '.quantity', $containerProduct->pivot->quantity) }}"
                                            :required="true" />
                                    </div>
                                    @if ($index > 0)
                                        <div class="col-md-1">
                                ...
                                <div class="form-group mt-2">
                                    <x-admin.form-select name="status" id="status" label="{{ __('Status') }}"
                                        ...
                                        @foreach ($statuses as $status)
                                            <x-admin.select-option :value="$status" :selected="old('status', $container->status) === $status" :text="$status" />
                                        @endforeach
                                    </x-admin.form-select>
                                </div>
                            </div>
...
@push('js')
    <script>
        let productIndex = {{ $container->products->count() }};
        ...
        $('#add_product_item').click(function() {
            ...
            <div class="row g-2 product-item align-items-end mt-2">
                ...
                <div class="col-md-1">
                    <button type="button" class="btn btn-danger btn-sm remove-product-item" title="{{ __('Remove') }}">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
