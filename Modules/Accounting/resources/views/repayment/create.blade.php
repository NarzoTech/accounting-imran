...
@section('title')
    <title>{{ __('Create Repayment') }}</title>
@endsection
@section('admin-content')
    <form action="{{ route('admin.repayment.store') }}" method="post" id="create_form" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <!-- Main Form Area -->
            <div class="col-md-7 col-lg-8">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h4 class="section_title">{{ __('Create Repayment') }}</h4>
                    </div>
                    <div class="card-body">
                        <!-- Investor Details -->
                            ...
                            <div class="col-md-12">
                                <x-admin.form-select name="investment_id" label="{{ __('Investment') }}" required="true">
                                    ...
                                    @foreach ($investor->investments as $investment)
                                        <x-admin.select-option :value="$investment->id" :text="__('Amount:') .
                                            ' ' .
